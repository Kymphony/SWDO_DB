...
public class FileService {
	...
	public static String saveFile(MultipartFile mfile, String uploadPath) {
		...
		//저장할 파일명을 오늘 날짜의 년월일로 생성(중복 방지를 위해 날자로 구분)
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");//파일이 업로드된 날자를 스트링으로 변환
		...
		while (true) {
			...
			//만에 하나 똑같은 시간에 똑같은 이름의 파일이 업로드 됐다면 while을 한번 더 돌려서
			//다른 시간대를 파일명에 붙이는 구조
		}		
		
		//파일 저장
		try {//transferTO: 한번에 데이터를 카피해주는 객체
